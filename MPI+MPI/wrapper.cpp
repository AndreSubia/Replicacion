	...
	struct comm_package* comm_handle)
{
	...
	{ //leader
		...
		{   
			...
			MPI_Allgatherv(local_addr, msize*(comm_handle->shmemcomm_size), data_type, start_addr, param_handle->recvcounts, param_handle->displs, data_type, comm_handle->bridge_comm);
			if(comm_handle->shmemcomm_size > 1){
				MPI_Barrier(comm_handle->shmem_comm);
			}
	    }
	...
	MPI_Datatype data_type, MPI_Op op, struct comm_package* comm_handle, MPI_Win win)
{
    ...
    if (comm_handle->shmem_comm != MPI_COMM_NULL){
		if (threshold_size <= SWITCH_MSG_BYTESIZE){ // method 2: serial but not sync
		  // This is for the operation of summation (MPI_SUM)
			...
			MPI_Reduce(start_addr + sharedmem_rank*msize, start_addr + bound_size, msize, data_type, op, 0, comm_handle->shmem_comm);
		}
		count += 1.0;
	}
